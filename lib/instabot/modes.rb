...
module Modes
  ...
  def auto_follow
    ...
    while @maximums[:follows_in_day] < @maximums[:max_follows_per_day]
      begin
        ...
        handle_user_informations_data(all_users[id])
        puts '[+] '.cyan + "Trying to follow user [#{all_users[id]}]"
        follow(@user_informations[:id])
  ...
  def auto_like
    ...
    while @maximums[:likes_in_day] < @maximums[:max_likes_per_day]
      begin
        handle_media_informations_data(all_medias[id])
        puts '[+] '.cyan + "Trying to like media [#{all_medias[id]}]"
        like(@media_informations[:id])
  ...
  def auto_comment
    ...
    while @maximums[:comments_in_day] < @maximums[:max_comments_per_day]
      begin
        handle_media_informations_data(all_medias[id])
        if @media_informations[:comments_disabled]
        ...
        comment(@media_informations[:id], generated_comment)
